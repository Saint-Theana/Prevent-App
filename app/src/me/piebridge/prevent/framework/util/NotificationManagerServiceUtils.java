        ...
        try {
            for (int flag : REMOVE_FLAGS) {
                if (length == 0x4) {
                    ...
                    cancelAllNotificationsInt.invoke(notificationManagerService, packageName, flag, 0, true, 0);
                } else if (length == 0x9) {
                    cancelAllNotificationsInt.invoke(notificationManagerService, uid, pid, packageName, flag, 0, true, 0, REASON_PACKAGE_CHANGED, null);
                }
            }
            return true;
